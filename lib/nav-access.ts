...
export function canShowNavHref(role: UserRole, href: string): boolean {
  ...
  switch (path) {
    ...
    case '/settings/billing': return canManageBilling(role);
    ...
    case '/portal/contractor': case '/portal/client': return false;
    default:
...
function canAccessSettingsPathByRole(role: UserRole, path: string): boolean {
  if (isClientRole(role) || isContractorRole(role)) return isPortalPersonalSettingsPath(path);
  if (path.startsWith('/settings/billing') || path.startsWith('/settings/ai-usage')) return canManageBilling(role);
  if (path.startsWith('/settings/referrals') || path.startsWith('/settings/branding') || path.startsWith('/settings/integrations') || path.startsWith('/settings/departments') || path.startsWith('/settings/api')) return canManageOrganizationSettings(role);
  if (path.startsWith('/settings/people') || path.startsWith('/settings/team')) return canViewTeam(role);
  if (path === '/settings' || path.startsWith('/settings?')) return canManageOrganizationSettings(role);
  if (path.startsWith('/settings/account') || path.startsWith('/settings/security') || path.startsWith('/settings/notifications') || path.startsWith('/settings/privacy')) return true;
  return false;
}
...
export function primaryNavHrefsForRole(role: UserRole): string[] {
  ...
  if (role === 'manager') return ['/dashboard', '/jobs', '/schedule', '/customers', '/people', '/expenses', '/reports', '/settings'];
