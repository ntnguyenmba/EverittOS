...
function paymentMethodLabel(method: string): string {
  const labels: Record<string, string> = {
    stripe: 'Stripe', square: 'Square', paypal: 'PayPal', venmo: 'Venmo', zelle: 'Zelle', cash_app: 'Cash App', custom: 'online'
  };
  return labels[method] || 'online';
}
...
function defaultSubject(docType: OutboundDocType, subject: string | null): string {
  ...
  const fallbacks: Record<OutboundDocType, string> = {
    review: 'We would love your feedback', proposal: 'Your proposal', estimate: 'Your estimate', invoice: 'Your invoice', message: 'Message from your service team', receipt: 'Payment receipt'
  };
...
function invoiceText(doc: OutboundDocument, body: string, payment: InvoicePaymentPreference): string {
  ...
  if (payment.link) lines.push(`Pay ${amount || 'invoice'}: ${payment.link}`);
...
function emailHtml(doc: OutboundDocument, payment: InvoicePaymentPreference): string {
  ...
  const body = escapeHtml(plainBody).replace(/\n/g, '<br />');
  const amount = doc.amount != null && doc.doc_type !== 'review' ? moneyLabel(doc.amount) : '';
  const payButton = doc.doc_type === 'invoice' && payment.link
    ? `<p style="margin:16px 0 0"><a href="${escapeHtml(payment.link)}" style="display:inline-block;background:#25364A;color:#ffffff;text-decoration:none;border-radius:10px;padding:13px 20px;font-weight:700">Pay ${escapeHtml(amount || 'invoice')}</a></p><p style="margin:8px 0 0;color:#667085;font-size:13px">Pay securely with ${escapeHtml(paymentMethodLabel(payment.method))}.</p>`
    : '';
  const instructions = doc.doc_type === 'invoice' && payment.instructions
    ? `<div style="margin:14px 0 0;padding:14px 16px;border-left:2px solid #A7ADA6;color:#46504B"><strong>Payment instructions</strong><br />${escapeHtml(payment.instructions).replace(/\n/g, '<br />')}</div>`
    : '';
  const invoiceBlock = doc.doc_type === 'invoice' && amount
    ? `<div style="margin:18px 0;padding:18px;border:1px solid #D9DED7;border-radius:14px;background:#F7F7F4"><p style="margin:0 0 6px;color:#66705F;font-size:13px;text-transform:uppercase;letter-spacing:.08em">Amount due</p><p style="margin:0;color:#24302B;font-size:28px;font-weight:800">${escapeHtml(amount)}</p>${payButton}${instructions}</div>`
    : '';
  const amountLine = amount && doc.doc_type !== 'invoice' ? `<p><strong>Amount:</strong> ${escapeHtml(amount)}</p>` : '';
  const feedbackButton = doc.doc_type === 'review' ? `<p><a href="${FEEDBACK_FORM_URL}" style="display:inline-block;background:#234A84;color:#ffffff;text-decoration:none;border-radius:999px;padding:12px 20px;font-weight:700">Share Feedback</a></p>` : '';
  const replyNote = doc.doc_type === 'invoice' ? '<p>Please reply to this email if you have any questions.</p>' : '';
  return `<div style="font-family:Inter,system-ui,sans-serif;line-height:1.6;color:#25364A;max-width:640px"><p>${body}</p>${invoiceBlock}${feedbackButton}${amountLine}${replyNote}</div>`;
}
...
export async function sendOutboundDocument(input: { supabase: SupabaseClient; organizationId: string; userId: string; document: OutboundDocument; }): Promise<SendOutboundResult> {
  ...
  if (!recipient) throw new Error('Recipient email is required before sending.');

  const payment: InvoicePaymentPreference = { method: '', link: '', instructions: '' };
  if (document.doc_type === 'invoice') {
    const { data: settings, error: settingsError } = await supabase.from('organization_settings').select('preferred_payment_method,payment_link,payment_instructions').eq('organization_id', organizationId).maybeSingle();
    if (settingsError && !isMissingSchemaError(settingsError)) console.warn('Invoice payment settings lookup failed:', settingsError.message);
    payment.method = String(settings?.preferred_payment_method || '');
    payment.link = safeHttpsUrl(String(settings?.payment_link || ''));
    payment.instructions = String(settings?.payment_instructions || '').trim();
  }

  const subject = defaultSubject(document.doc_type, document.subject);
  const body = document.body?.trim() || '';
  const textBody = document.doc_type === 'invoice' ? invoiceText(document, body, payment) : body;
  ...
  if (!transactionalEmailConfigured()) {
    ...
    const result = await sendTransactionalEmail({ to: recipient, subject, html: emailHtml(document, payment), text: textBody });
    emailSent = result.sent; deliveryProvider = result.provider; externalId = result.id;
    if (!result.sent) failureReason = result.error || 'Email delivery failed';
  }

  const status = emailSent ? 'sent' : 'failed';
  const deliveryNote = emailSent ? undefined : failureReason || 'Email delivery failed. Check the outbound send history for details.';
  ...
  if (error || !updated) {
    if (isMissingSchemaError(error)) throw new Error('Outbound tables are not set up yet. Run supabase/manual_schema_repair.sql in the Supabase SQL Editor.');
  ...
  if (historyResult.error && !isMissingSchemaError(historyResult.error)) console.warn('outbound_sent_history insert failed:', historyResult.error.message);
