...
export async function requireCalendarImportManager(): Promise<CalendarImportAuth> {
  ...
  if (!user) {
    return {
      ok: false,
      response: NextResponse.json({ error: 'Unauthorized' }, { status: 401, headers: CALENDAR_IMPORT_NO_CACHE })
    };
  }
  ...
  if (!org || !canManageOrganizationSettings(normalizeRole(org.role))) {
    return {
      ok: false,
      response: NextResponse.json({ error: 'Permission denied' }, { status: 403, headers: CALENDAR_IMPORT_NO_CACHE })
    };
  }
  ...
  if (!admin) {
    return {
      ok: false,
      response: NextResponse.json({ error: 'Server not configured' }, { status: 503, headers: CALENDAR_IMPORT_NO_CACHE })
    };
  }
