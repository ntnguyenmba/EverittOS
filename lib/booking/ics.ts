...
export function generateBookingIcs(input: BookingIcsInput): string {
  ...
  const useLocalTime = Boolean(
    timeZone && !hasExplicitTimeZone(input.startsAt) && !hasExplicitTimeZone(input.endsAt)
  );
  const dtStart = useLocalTime ? formatLocalIcsDate(input.startsAt) : formatUtcIcsDate(input.startsAt);
  const dtEnd = useLocalTime ? formatLocalIcsDate(input.endsAt) : formatUtcIcsDate(input.endsAt);
