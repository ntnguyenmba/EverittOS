...
async function withWorkspaceContext(
  ...
  if (!profile) return null;
  const workspace = await resolveMiddlewareWorkspaceContext(
    client,
    userId,
...
async function fetchProfileBillingFields(
  ...
  const { data: planRow, error: planError } = await client
    ...
    .eq('id', userId)
    .maybeSingle();
  if (!planError && planRow?.plan?.trim()) billing.plan = planRow.plan;
  ...
  const { data: statusRow, error: statusError } = await client
    ...
    .eq('id', userId)
    .maybeSingle();
  if (!statusError && statusRow?.subscription_status?.trim()) billing.subscription_status = statusRow.subscription_status;
...
export async function fetchProfileByUserId(
  ...
  if (!fullError) {
    const profile = await withWorkspaceContext(client, userId, withBillingDefaults(fullProfile as ProfileRow));
    return { profile, usedCoreSelect: false };
  }
  ...
  const profile = await withWorkspaceContext(
    client,
    userId,
    withBillingDefaults({ ...(coreProfile as ProfileRow), ...billing })
  );
...
export async function resolveProfilePlan(
  ...
  if (organizationId) {
    ...
    if (orgSub?.plan) return normalizePlan(orgSub.plan);
    ...
    if (org?.owner_user_id && org.owner_user_id !== userId) {
      const { data: ownerProfile } = await client
        ...
        .maybeSingle();
      if (ownerProfile?.plan?.trim()) return normalizePlan(ownerProfile.plan);
    }
  }

  if (profile?.plan?.trim()) return normalizePlan(profile.plan);
  return 'free';
}
...
export async function resolveProfileSubscriptionStatus(
  ...
  if (profile?.subscription_status?.trim()) return profile.subscription_status;
  ...
  if (!error && subscription?.status) return subscription.status;
  return 'free';
}
...
export async function upsertProfileRow(
  ...
  if (!fullError) return {};
  if (!isMissingColumnError(fullError.message)) return { error: fullError.message };
...
export async function updateProfileLink(
  ...
  if (!error) return { profile: withBillingDefaults(data as ProfileRow) };
  if (!isMissingColumnError(error.message)) return { profile: null, error: error.message };
