...
export default function JobDetailPage() {
  ...
  async function loadJob() {
    ...
    if (limitsForPlan(userPlan).crewAssignment) {
      let workersQuery = supabase.from('workers').select('id, name, worker_type, auth_user_id').eq('active', true).order('name');
      workersQuery = org?.organizationId ? workersQuery.eq('organization_id', org.organizationId) : workersQuery.eq('user_id', user.id);
      const [{ data: crew }, { data: memberRoles }] = await Promise.all([
        workersQuery,
        org?.organizationId
