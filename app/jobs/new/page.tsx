...
export default function NewJobPage() {
  ...
  const [plan, setPlan] = useState<EverittosPlan>('free'); const [role, setRole] = useState<UserRole>('employee'); const [authorized, setAuthorized] = useState(false);

  useEffect(() => { async function load() {
    const { data: { user } } = await supabase.auth.getUser(); if (!user) { router.replace('/login?next=/jobs/new'); return; }
    const [{ data: profile }, membershipsResponse] = await Promise.all([supabase.from('profiles').select('plan').eq('id', user.id).maybeSingle(), fetch('/api/org/memberships', { cache: 'no-store' })]);
    setPlan(normalizePlan(profile?.plan)); if (!membershipsResponse.ok) { router.replace('/dashboard'); return; }
    const memberships = (await membershipsResponse.json()) as MembershipResponse; const activeRole = normalizeRole(memberships.activeRole || 'employee'); setRole(activeRole);
  ...
  return <AppShell plan={plan} role={role}><PageHeader title={t('dashboard.newJob')} /><div className={styles.formWrap}><Suspense fallback={<p className="loading-state">{t('common.loading')}</p>}><JobPrefillBridge /><JobCreator onJobCreated={(jobId) => void finishJobCreation(jobId)} /><JobContractorOptions /></Suspense></div></AppShell>;
}
