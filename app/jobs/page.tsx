...
const copy = {
  en: {
    ...
    filtered: 'Filtered',
    showAll: 'Show all',
    missingFinish: 'Finished jobs missing a finish date.',
    loading: 'Loading…',
    unableLoad: 'Unable to load jobs.',
    removeConfirm: 'Remove job "{title}"?',
    unableRemove: 'Unable to remove job.',
  ...
  es: {
    ...
    missingFinish: 'Trabajos finalizados sin fecha de finalización.',
    loading: 'Cargando…',
    unableLoad: 'No se pudieron cargar los trabajos.',
    removeConfirm: '¿Eliminar el trabajo "{title}"?',
    unableRemove: 'No se pudo eliminar el trabajo.',
    ...
    unassigned: 'Necesita trabajador'
  },
  vi: {
    ...
    missingFinish: 'Công việc đã hoàn thành nhưng thiếu ngày hoàn tất.',
    loading: 'Đang tải…',
    unableLoad: 'Không thể tải công việc.',
    removeConfirm: 'Xóa công việc "{title}"?',
    unableRemove: 'Không thể xóa công việc.',
...
function jobCityState(address: string | null | undefined): string {
  const parts = parseAddressParts(address || null);
  return parts.cityState;
}

function filterTabClass(active: boolean, alert = false) {
  return ['jobs-filter-tab', active ? 'jobs-filter-tab-active' : '', alert ? 'jobs-filter-tab-alert' : '']
    .filter(Boolean)
    .join(' ');
}
...
function JobsList() {
  ...
  const exportQuery = useMemo(() => {
    ...
    return params;
  }, [customerFilter, statusFilter, periodFilter, assignmentFilter, assignedToFilter, createdFromFilter]);
    ...
    <AppShell plan={plan} role={role} className="jobs-shell-minimal">
      <div className="jobs-list-page">
        ...
        <div className="jobs-filter-tabs" aria-label="Job filters">
          <Link href="/jobs" className={filterTabClass(activeAll)} aria-current={activeAll ? 'page' : undefined}>
            {c.all}
          </Link>
          <Link
            href="/jobs?period=today"
            className={filterTabClass(activeToday)}
            aria-current={activeToday ? 'page' : undefined}
          >
            {c.today}
          </Link>
          <Link
            href="/jobs?status=active"
            className={filterTabClass(activeActive)}
            aria-current={activeActive ? 'page' : undefined}
          >
            {c.active}
          </Link>
          <Link
            href="/jobs?status=finished"
            className={filterTabClass(activeFinished)}
            aria-current={activeFinished ? 'page' : undefined}
          >
            {c.finished}
          </Link>
          {managerView ? (
            <Link
              href="/jobs?filter=unassigned"
              className={filterTabClass(activeNeedsWorker, true)}
              aria-current={activeNeedsWorker ? 'page' : undefined}
            >
              {c.needsWorker}
            </Link>
          ) : null}
        ...
        {filtered ? (
          <p className="muted" style={{ marginBottom: 12 }}>
            {c.filtered} · <Link href="/jobs">{c.showAll}</Link>
          </p>
        ) : null}
          ...
          <div className="card jobs-table-card">
            <div className="jobs-mobile-table-wrap">
              <table className="jobs-operations-table jobs-mobile-table">
                <thead>
                  <tr>
                    {[c.date, c.job, c.assignedTo, c.status, c.actions].map((label) => (
                ...
                <tbody>
                  {rows.map((job) => {
                    ...
                    return (
                      ...
                      >
                        <td data-label={c.date} className="jobs-col-date">
                          <strong>{formatDate(job, localeCode, c.unscheduled)}</strong>
                          <span className="jobs-row-time">{formatTime(job, localeCode)}</span>
                        </td>
                        <td data-label={c.job} className="jobs-col-property">
                          <Link
                            href={`/jobs/${job.id}`}
                            onClick={(event) => event.stopPropagation()}
                            className="jobs-property-link"
                          >
                            {job.title}
                          </Link>
                          <div className="jobs-secondary">{job.customer_name || c.noCustomer}</div>
                          {cityState ? <div className="jobs-secondary">{cityState}</div> : null}
                          {job.photo_count ? (
                            <div className="jobs-photo-count">
                              {job.photo_count} {job.photo_count === 1 ? c.photo : c.photos}
                            </div>
                          ) : null}
                        </td>
                        <td data-label={c.assignedTo} className="jobs-col-assigned">
                          <span className={needsWorker ? 'jobs-needs-worker' : undefined}>{assignment}</span>
                        </td>
                        <td data-label={c.status} className="jobs-col-status">
                          <StatusPill status={job.status} />
                        </td>
                        <td
                          data-label={c.actions}
                          className="jobs-col-actions"
                          onClick={(event) => event.stopPropagation()}
                        >
                          <div className="jobs-more-menu" data-jobs-menu={job.id}>
                            <button
                              type="button"
                              ...
                              onClick={(event) => {
                                ...
                                setOpenMenuId(menuOpen ? '' : job.id);
                              }}
                            >
                              •••
                            </button>
                            {menuOpen ? (
                              <div className="jobs-more-panel" role="menu">
                                <Link
                                  href={`/jobs/${job.id}`}
                                ...
                                {managerView ? (
                                  <button
                                    type="button"
                                    className="jobs-menu-item jobs-menu-danger"
                                    role="menuitem"
