...
export default function RefundPolicyPage() {
  ...
  return <main className="section"><div className="container legal-document" style={{ maxWidth: 720 }}><h2>{c.title}</h2><p className="muted">{c.part} <Link href="/terms">{c.terms}</Link> · {c.version} {TERMS_VERSION} · {c.updated}</p><p>{c.body}</p><h3>{c.cancelTitle}</h3><p>{c.cancel}</p><h3>{c.relatedTitle}</h3><p><Link href="/terms">{c.terms}</Link> · <Link href="/privacy">{c.privacy}</Link> · <Link href="/settings/billing">{c.plans}</Link></p><h3>{c.contact}</h3><p>{c.billing}: <a href={`mailto:${SUPPORT_EMAIL}`}>{SUPPORT_EMAIL}</a></p><LegalNotice /><Link className="btn" href="/login">{c.back}</Link></div></main>;
}
