...
const dashboardCopy = {
  en: { ownerSubtitle: 'Your next job and the work that needs attention today.', staffSubtitle: 'Your assigned work and schedule for today.', managerSubtitle: 'Today’s team workload and priorities.', todaysWork: "Today's work", loadError: 'Some information could not load.', loading: 'Loading…', retry: 'Retry', todaysJobs: "Today's Jobs", jobsNeedingAttention: 'Needs Attention', openLeads: 'Open Leads', myWork: 'My work', myJobs: 'My jobs', schedule: 'Schedule', newJob: 'New Job', nextJob: 'Next job', noUpcomingJob: 'No upcoming job', openJob: 'Open job' },
  es: { ownerSubtitle: 'Tu próximo trabajo y lo que requiere atención hoy.', staffSubtitle: 'Tu trabajo asignado y horario de hoy.', managerSubtitle: 'La carga de trabajo y las prioridades del equipo para hoy.', todaysWork: 'Trabajo de hoy', loadError: 'No se pudo cargar parte de la información.', loading: 'Cargando…', retry: 'Reintentar', todaysJobs: 'Trabajos de hoy', jobsNeedingAttention: 'Necesita atención', openLeads: 'Prospectos abiertos', myWork: 'Mi trabajo', myJobs: 'Mis trabajos', schedule: 'Calendario', newJob: 'Nuevo trabajo', nextJob: 'Próximo trabajo', noUpcomingJob: 'No hay trabajo próximo', openJob: 'Abrir trabajo' },
  vi: { ownerSubtitle: 'Công việc tiếp theo và những việc cần chú ý hôm nay.', staffSubtitle: 'Công việc được giao và lịch của bạn hôm nay.', managerSubtitle: 'Khối lượng công việc và ưu tiên của nhóm hôm nay.', todaysWork: 'Công việc hôm nay', loadError: 'Một số thông tin không thể tải.', loading: 'Đang tải…', retry: 'Thử lại', todaysJobs: 'Công việc hôm nay', jobsNeedingAttention: 'Cần chú ý', openLeads: 'Khách tiềm năng đang mở', myWork: 'Công việc của tôi', myJobs: 'Công việc của tôi', schedule: 'Lịch', newJob: 'Công việc mới', nextJob: 'Công việc tiếp theo', noUpcomingJob: 'Không có công việc sắp tới', openJob: 'Mở công việc' }
} as const;
...
export default function DashboardPage() {
  ...
  async function loadDashboard() {
    ...
    if (!userId) {
      const session = await withTimeout(supabase.auth.getSession(), { data: { session: null }, error: new Error('Session timed out') }) as { data: { session: { user: { id: string } } | null }; error: unknown };
