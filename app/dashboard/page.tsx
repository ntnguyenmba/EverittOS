...
const dashboardCopy = {
  en: { todaysWork: "Today's work", loadError: 'Some information could not load.', loading: 'Loading…', retry: 'Retry', todaysJobs: "Today's Jobs", jobsNeedingAttention: 'Needs Attention', openLeads: 'Open Leads', myWork: 'My work', myJobs: 'My jobs', schedule: 'Schedule', newJob: 'New Job', quotes: 'Quotes', nextJob: 'Next job', noUpcomingJob: 'No upcoming job', quotesWaiting: 'Quotes waiting', moneyLate: 'Money late', openJob: 'Open job' },
  es: { todaysWork: 'Trabajo de hoy', loadError: 'No se pudo cargar parte de la información.', loading: 'Cargando…', retry: 'Reintentar', todaysJobs: 'Trabajos de hoy', jobsNeedingAttention: 'Necesita atención', openLeads: 'Prospectos abiertos', myWork: 'Mi trabajo', myJobs: 'Mis trabajos', schedule: 'Calendario', newJob: 'Nuevo trabajo', quotes: 'Cotizaciones', nextJob: 'Próximo trabajo', noUpcomingJob: 'No hay trabajo próximo', quotesWaiting: 'Cotizaciones pendientes', moneyLate: 'Dinero atrasado', openJob: 'Abrir trabajo' },
  vi: { todaysWork: 'Công việc hôm nay', loadError: 'Một số thông tin không thể tải.', loading: 'Đang tải…', retry: 'Thử lại', todaysJobs: 'Công việc hôm nay', jobsNeedingAttention: 'Cần chú ý', openLeads: 'Khách tiềm năng đang mở', myWork: 'Công việc của tôi', myJobs: 'Công việc của tôi', schedule: 'Lịch', newJob: 'Công việc mới', quotes: 'Báo giá', nextJob: 'Công việc tiếp theo', noUpcomingJob: 'Không có công việc sắp tới', quotesWaiting: 'Báo giá đang chờ', moneyLate: 'Tiền quá hạn', openJob: 'Mở công việc' }
} as const;
...
type DashboardJob = JobCountRow & { id: string; title?: string | null; customer_name?: string | null; address?: string | null; assigned_to?: string | null; scheduled_start?: string | null; start_date?: string | null; due_date?: string | null; is_skipped?: boolean | null; };
...
export default function DashboardPage() {
  ...
  const [plan, setPlan] = useState<EverittosPlan>('free'); const [role, setRole] = useState<UserRole>('owner'); const [ready, setReady] = useState(false); const [loading, setLoading] = useState(true); const [loadError, setLoadError] = useState(false); const [revenue, setRevenue] = useState<DashboardRevenueMetrics>(emptyRevenue); const [ops, setOps] = useState<OpsCounts>({ todayJobs: 0, needsAttention: 0, openLeads: 0, singleOpenLeadId: null, quotesWaiting: 0, nextJob: null });
  async function loadDashboard() {
    setLoading(true); setLoadError(false);
    const auth = await withTimeout<{ data: { user: { id: string } | null }; error: Error | null }>(supabase.auth.getUser(), { data: { user: null }, error: new Error('Authentication timed out') }, 5000); const user = auth.data.user; if (!user) { router.replace('/login'); return; }
    type ProfileRow = { plan?: string | null; role?: string | null };
    const [profileResult, organization] = await Promise.all([withTimeout<{ data: ProfileRow | null; error: Error | null }>(supabase.from('profiles').select('plan, role').eq('id', user.id).maybeSingle(), { data: null, error: new Error('Profile timed out') }, 3500), withTimeout(ensureOrganizationForUser(user.id), null, 3500)]);
    const nextPlan = normalizePlan(profileResult.data?.plan); const nextRole = normalizeRole(organization?.role || profileResult.data?.role); if (isClientRole(nextRole)) { router.replace('/portal/client'); return; } if (isContractorRole(nextRole)) { router.replace('/portal/contractor'); return; }
    setPlan(nextPlan); setRole(nextRole); setReady(true);
    const organizationId = organization?.organizationId || null; const scopeColumn = organizationId ? 'organization_id' : 'user_id'; const scopeValue = organizationId || user.id; const today = formatLocalDate(new Date()); const ownerFinance = canAccessFinancials(nextRole, nextPlan);
    ...
    const [nextRevenue, jobsResult, customersResult, quotesResult] = await Promise.all([ownerFinance ? withTimeout(fetchDashboardRevenueMetrics(supabase, organizationId), { ...emptyRevenue, loadFailed: true }) : Promise.resolve(emptyRevenue), withTimeout(supabase.from('jobs').select('id, title, customer_name, address, status, start_date, due_date, scheduled_start, completed_at, created_at, assigned_to, is_skipped, recurring_series_id, occurrence_date').eq(scopeColumn, scopeValue).limit(5000), { data: [], error: new Error('Jobs timed out') }), withTimeout(supabase.from('customers').select('id, record_type, pipeline_stage').eq(scopeColumn, scopeValue).limit(10000), { data: [], error: new Error('Customers timed out') }), quotesTask]);
    const jobs = (jobsResult.data || []) as DashboardJob[]; const customers = (customersResult.data || []) as Array<{ id: string; record_type: string | null; pipeline_stage: string | null }>; const todayJobs = filterValidJobsInPeriod(jobs, 'today'); const activeJobs = jobs.filter((job) => !['completed', 'complete', 'done', 'finished', 'closed', 'cancelled', 'canceled', 'draft', 'skipped'].includes(String(job.status || '').toLowerCase()) && !job.is_skipped); const needsAttention = activeJobs.filter((job) => { const status = String(job.status || '').toLowerCase(); const due = (job.due_date || '').slice(0, 10); return !job.assigned_to || status === 'new' || Boolean(due && due < today); }).length; const openLeadRows = customers.filter((row) => row.record_type === 'lead' && !['won', 'closed_lost', 'cancelled', 'lost'].includes(row.pipeline_stage || 'open')); const nextJobRow = activeJobs.filter((job) => { const date = jobDateValue(job); return date && formatLocalDate(date) >= today; }).sort((a, b) => (jobDateValue(a)?.getTime() || Number.MAX_SAFE_INTEGER) - (jobDateValue(b)?.getTime() || Number.MAX_SAFE_INTEGER))[0] || null;
    setRevenue(nextRevenue); setOps({ todayJobs: todayJobs.length, needsAttention, openLeads: openLeadRows.length, singleOpenLeadId: openLeadRows.length === 1 ? openLeadRows[0].id : null, quotesWaiting: Array.isArray(quotesResult.data) ? quotesResult.data.length : 0, nextJob: nextJobRow ? { id: nextJobRow.id, title: String(nextJobRow.title || nextJobRow.customer_name || c.nextJob), detail: formatNextJobDetail(nextJobRow, locale === 'vi' ? 'vi-VN' : locale === 'es' ? 'es-US' : 'en-US') } : null }); setLoadError(Boolean(profileResult.error || jobsResult.error || customersResult.error || nextRevenue.loadFailed)); setLoading(false);
  }
  useEffect(() => { void loadDashboard(); const onFocus = () => void loadDashboard(); const onVisible = () => { if (document.visibilityState === 'visible') void loadDashboard(); }; window.addEventListener('focus', onFocus); document.addEventListener('visibilitychange', onVisible); return () => { window.removeEventListener('focus', onFocus); document.removeEventListener('visibilitychange', onVisible); }; }, []);
