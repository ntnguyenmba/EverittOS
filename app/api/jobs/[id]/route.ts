...
async function jobsHaveFinancialHistory(
  ...
  const [invoices, payments, labor, expenses] = await Promise.all([
    ctx.supabase.from('invoices').select('id').eq('organization_id', orgId).in('job_id', jobIds).limit(1),
    ctx.supabase.from('job_payments').select('id').eq('organization_id', orgId).in('job_id', jobIds).limit(1),
    ctx.supabase.from('job_labor').select('id').eq('organization_id', orgId).in('job_id', jobIds).limit(1),
    ctx.supabase.from('expenses').select('id').eq('organization_id', orgId).in('job_id', jobIds).limit(1)
  ]);
  return Boolean(invoices.data?.length || payments.data?.length || labor.data?.length || expenses.data?.length);
}
...
export async function DELETE(_request: Request, context: RouteContext) {
  ...
  if (rpc.error && !/could not find|does not exist|schema cache|function/i.test(rpc.error.message || '')) {
    return permanentDeleteErrorResponse(rpc.error.message || '');
  }
