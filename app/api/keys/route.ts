...
export async function GET(request: Request) {
  ...
  return NextResponse.json({ keys: keys.map((k) => ({ id: k.id, name: k.name, key_prefix: k.key_prefix, scopes: k.scopes, created_at: k.created_at, last_used_at: k.last_used_at, revoked_at: k.revoked_at })) });
}
...
export async function POST(request: Request) {
  ...
  if (!body.name?.trim()) return NextResponse.json({ error: c.apiKeyName }, { status: 400 });

  const admin = createAdminSupabase();
  if (!admin) return NextResponse.json({ error: c.serverUnavailable }, { status: 503 });

  const { row, rawKey } = await createApiKey(admin, { organizationId: org.organizationId, name: body.name, createdBy: user.id });
  return NextResponse.json({ key: { id: row.id, name: row.name, key_prefix: row.key_prefix, created_at: row.created_at }, rawKey });
}
