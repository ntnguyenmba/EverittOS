...
import { loadContractorPortalJobsExport } from '@/lib/exports/portal-export-data';
...
export async function GET(request: Request) {
  ...
  const headers = columnHeaders(CONTRACTOR_JOBS_COLUMNS);
  const tableRows = data.rows.map((row) => stringRowValues(CONTRACTOR_JOBS_COLUMNS, row));
