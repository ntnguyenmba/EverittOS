...
export async function POST(request: Request) {
  ...
  if (price === null) return NextResponse.json({ error: 'Choose a valid quote price.' }, { status: 400 });
  ...
  const row = {
    ...
    status: 'draft',
    service_type: String(body.serviceType || '').trim() || null,
    size_value: optionalNumber(body.sizeValue),
    size_unit: String(body.sizeUnit || '').trim() || null,
    primary_units: optionalNumber(body.primaryUnits),
    extra_units: optionalNumber(body.extraUnits),
    condition: String(body.condition || '').trim() || null,
    frequency: String(body.frequency || '').trim() || null,
    add_ons: Array.isArray(body.addOns) ? body.addOns.map(String).slice(0, 30) : [],
    labor_hours: optionalNumber(body.laborHours),
    price,
    currency: String(body.currency || 'USD').trim().slice(0, 8) || 'USD',
    source_request: String(body.sourceRequest || '').trim().slice(0, 5000) || null,
    customer_name: String(body.customerName || '').trim().slice(0, 200) || null,
    customer_email: String(body.customerEmail || '').trim().slice(0, 320) || null,
    customer_phone: String(body.customerPhone || '').trim().slice(0, 80) || null,
