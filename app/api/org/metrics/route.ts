...
export async function GET() {
  ...
  const [
    ...
    supabase
      ...
      .select('id', { count: 'exact', head: true })
      .eq('organization_id', orgId)
      .eq('event_name', 'client_portal_view')
  ...
  const reportCompletionRatePct =
    eligibleJobs.length === 0 ? 0 : Math.round((jobsWithReports.size / eligibleJobs.length) * 100);

  const mrrUsd = PLAN_MRR[plan] || 0;

  const subscriptionBreakdown: Record<string, number> = {};
  (subsRes.data || []).forEach((p) => {
    const key = normalizePlan(p.plan);
    subscriptionBreakdown[key] = (subscriptionBreakdown[key] || 0) + 1;
  });

  const months = lastSixMonths();
  const jobsTrend = months.map((m) => ({
    label: monthKey(m),
    value: jobs.filter((j) => {
  ...
  const growthTrend = months.map((m) => ({
    ...
    value: members.filter((member) => {
      ...
      return d <= new Date(m.getFullYear(), m.getMonth() + 1, 0);
    }).length
  }));
  ...
  return NextResponse.json({
    ...
    arrUsd: mrrUsd * 12,
    activeUsers,
    monthlyJobs: jobsMonthRes.count || 0,
    technicianUtilizationPct,
    revenueGrowthPct,
    clientPortalViews30d: portalEventsRes.count || 0,
