...
export async function GET() {
  ...
  if (admin && org) {
    ...
    try {
      ...
      const optionalPromoColumnMissing = message.includes('stripe_promotion_code') && (message.includes('does not exist') || message.includes('42703'));
      if (!optionalPromoColumnMissing) console.warn('[ai-status] optional EVERITTTEAM lookup failed', message);
    }
    ...
    if (everittteamApplies) {
      ...
      everittteam = { applies: true, budgetUsd: pool.budgetUsd, usedUsd: pool.usedUsd, remainingUsd: pool.remainingUsd, percentUsed: pool.percentUsed, periodStart: pool.periodStart, ownerBypass, budgetExhausted, warning };
    }
    ...
    if (isStaffRole(role)) {
      ...
      const [dailyUsed, monthlyUsed] = staffLimitsApply ? await Promise.all([getDailyAiPromptCount(admin, user.id), getMonthlyAiPromptCount(admin, user.id)]) : [0, 0];
      staffAi = { applies: staffLimitsApply, dailyUsed, dailyCap: STAFF_DAILY_AI_PROMPT_LIMIT, monthlyUsed, monthlyCap: STAFF_MONTHLY_AI_PROMPT_LIMIT, workspaceStaffSpendUsd: summary.workspaceStaffSpendUsd, usersThisMonth: summary.staffUsersThisMonth };
    }
  }
