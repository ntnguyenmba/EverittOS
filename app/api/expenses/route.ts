...
export async function GET(request: Request) {
  ...
  if (search) {
    ...
    if (escaped) query = query.or(`vendor.ilike.%${escaped}%,description.ilike.%${escaped}%,notes.ilike.%${escaped}%`);
  }
  ...
  const expenses = await Promise.all((data || []).map(async (row) => {
    if (!row.receipt_url || !admin) return row;
    const { data: signed } = await admin.storage.from('expense-receipts').createSignedUrl(row.receipt_url, 3600);
