...
public class EverittFieldStorePlugin extends Plugin {
    ...
    private boolean allowedPhotoFile(File file) throws Exception { String base=jobsDir().getCanonicalPath()+File.separator;return file.getCanonicalPath().startsWith(base); }
    ...
    @PluginMethod public void savePhoto(PluginCall call) { String key=call.getString("key"),base64=call.getString("base64");if(key==null||base64==null){call.reject("Invalid photo.");return;}try{File file=new File(jobsDir(),safe(key)+"-"+UUID.randomUUID()+".jpg");try(FileOutputStream out=new FileOutputStream(file)){out.write(Base64.decode(base64,Base64.DEFAULT));}JSObject result=new JSObject();result.put("path",file.getAbsolutePath());call.resolve(result);}catch(Exception error){call.reject("Could not save photo.");} }
    @PluginMethod public void readPhoto(PluginCall call) { String path=call.getString("path");if(path==null){call.reject("Invalid photo path.");return;}try{File file=new File(path);if(!allowedPhotoFile(file)){call.reject("Invalid photo path.");return;}if(!file.exists()){JSObject result=new JSObject();result.put("base64",null);call.resolve(result);return;}ByteArrayOutputStream bytes=new ByteArrayOutputStream();try(FileInputStream in=new FileInputStream(file)){byte[] buffer=new byte[8192];int count;while((count=in.read(buffer))!=-1)bytes.write(buffer,0,count);}JSObject result=new JSObject();result.put("base64",Base64.encodeToString(bytes.toByteArray(),Base64.NO_WRAP));call.resolve(result);}catch(Exception error){call.reject("Could not read photo.");} }
