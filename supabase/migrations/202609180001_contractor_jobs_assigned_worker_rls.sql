-- Restore contractor read/update on jobs when assigned_to stores workers.id
-- (canonical write path) or via job_assignments / legacy auth.uid() assignment.
--
-- Later team-work repairs (20260901 / 20260906 / 20260907) dropped
-- public.is_assigned_to_job(id) from jobs_team_work_* policies and only kept
-- assigned_to = auth.uid(), which hides contractor-assigned jobs and zeroes
-- dashboard / my-work metrics under RLS.
