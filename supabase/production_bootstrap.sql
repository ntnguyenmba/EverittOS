-- =============================================================================
-- EverittOS PRODUCTION bootstrap (idempotent)
-- Project: Everitt OS (wxtatzlirexrwjehqgak)
--
-- Run ONCE in Supabase SQL Editor on the EXISTING legacy database.
-- Upgrades original schema (profiles, jobs, customers, invoices, workers,
-- technicians, job_photos, activity_events, ai_generations, companies) to
-- current production requirements without dropping or renaming legacy tables.
--
-- Rules: CREATE IF NOT EXISTS, ADD COLUMN IF NOT EXISTS, CREATE OR REPLACE
-- functions, policies created only when missing (legacy policies dropped first).
-- =============================================================================
...
begin
  if exists (
    ...
    raise notice 'Dropped incompatible business_profiles without user_id.';
  end if;
end $$;

-- Bridge legacy job_photos url columns to storage_path (current app expects storage_path)
do $$
begin
  if not exists (
    select 1 from information_schema.tables
  ...
  if exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'job_photos' and column_name = 'url'
  ) then
    execute $sql$
  ...
  if exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'job_photos' and column_name = 'photo_url'
  ) then
    execute $sql$
      update public.job_photos
      set storage_path = photo_url
      where storage_path is null and photo_url is not null
    $sql$;
  end if;
  if exists (
    select 1 from information_schema.columns
    ...
    execute $sql$
      ...
      where storage_path is null and path is not null
    $sql$;
  end if;
exception when others then
  raise notice 'job_photos storage_path bridge skipped: %', sqlerrm;
end $$;

-- Pull business names from legacy companies table when present
do $$
begin
  if exists (
    select 1 from information_schema.tables
    ...
    execute $sql$
      ...
      set business_name = coalesce(p.business_name, c.name)
      from public.companies c
      where c.user_id = p.id and (p.business_name is null or p.business_name = '')
    $sql$;
  elsif exists (
    select 1 from information_schema.tables
    where table_schema = 'public' and table_name = 'companies'
    ...
    execute $sql$
      update public.profiles p
      set business_name = coalesce(p.business_name, c.company_name)
      from public.companies c
      where c.owner_id = p.id and (p.business_name is null or p.business_name = '')
    $sql$;
  end if;
exception when others then
  raise notice 'companies profile backfill skipped: %', sqlerrm;
end $$;
...
-- Legacy invoices table: reuse existing table, add columns for client portal
do $$
begin
  if exists (
    select 1 from information_schema.tables
...
exception when others then
  raise notice 'Legacy invoices upgrade skipped: %', sqlerrm;
end $$;
...
begin
  if public._table_exists('profiles') then
    if public._col_exists('profiles', 'plan') then
      ...
      execute $sql$ update public.profiles set plan = 'pro' where lower(plan) = 'starter' $sql$;
      execute $sql$ update public.profiles set plan = 'growth' where plan = 'operations' $sql$;
      execute $sql$
...
alter table public.profiles add constraint profiles_role_check
  check (role in ('owner', 'admin', 'manager', 'employee', 'contractor', 'crew_lead', 'staff', 'client', 'viewer'));
...
alter table public.organization_members add constraint organization_members_role_check
  check (role in ('owner', 'admin', 'manager', 'employee', 'contractor', 'client', 'crew_lead', 'staff', 'viewer'));
...
alter table public.organization_invitations add constraint organization_invitations_role_check
  check (role in ('admin', 'manager', 'employee', 'contractor', 'client', 'crew_lead', 'staff', 'viewer'));
...
-- ---------------------------------------------------------------------------
-- Drop legacy owner-only policies from the original schema (safe if absent)
...
-- ---------------------------------------------------------------------------
-- 13. Backfill business_profiles, organizations, and organization_id columns
-- ---------------------------------------------------------------------------
insert into public.business_profiles (user_id, business_name, email, phone)
select p.id, p.business_name, p.email, p.phone
...
begin
  for r in
    select p.id as uid, coalesce(nullif(p.business_name, ''), bp.business_name, 'My Business') as bname
    from public.profiles p
