-- =============================================================================
-- EverittOS legacy database bootstrap (idempotent)
-- Run once in Supabase SQL Editor to upgrade the ORIGINAL EverittOS schema:
--   profiles, jobs, customers, invoices, workers, technicians, job_photos,
--   activity_events, ai_generations, companies
--
-- Creates all current production tables/columns (business_profiles, organizations,
-- organization_members, everittos_subscriptions, job_reports, activity_logs, etc.)
-- without dropping legacy tables or assuming newer migrations were applied.
--
-- Safe to re-run: IF NOT EXISTS / ADD COLUMN IF NOT EXISTS / DROP POLICY IF EXISTS.
...
-- Done — legacy tables (invoices, technicians, activity_events, ai_generations, companies) are untouched
select 'EverittOS legacy bootstrap complete' as status;
