...
export function AppShell({ plan, role, className, children }: AppShellProps) {
  ...
  return (
    <div className={shellClass}>
      ...
      <style jsx global>{`
        :root { --navy-primary:#243f53; --navy-secondary:#1d3343; --accent:#285d78; --accent-strong:#243f53; --secondary-blue:#285d78; --selected:#eef3f5; --warm-white:#ffffff; --soft-white:#ffffff; --bg:#e9eef2; }
        ...
        .dashboard-shell-overlay{position:fixed;inset:0;z-index:1;pointer-events:none;background:rgba(19,36,51,.58)}
        ...
        .dashboard-shell-header{position:sticky;top:0;z-index:42;display:block;width:100%;max-width:100%;padding:max(8px,env(safe-area-inset-top)) max(14px,env(safe-area-inset-right)) 8px max(14px,env(safe-area-inset-left));box-sizing:border-box;background:#243f53;border-bottom:1px solid rgba(255,255,255,.08)}
        ...
        @media(max-width:760px){
          ...
          .dashboard-shell-header{position:sticky!important;top:0!important;z-index:100!important;padding:max(8px,env(safe-area-inset-top)) max(14px,env(safe-area-inset-right)) 8px max(14px,env(safe-area-inset-left))!important;background:#243f53!important;backdrop-filter:none!important;-webkit-backdrop-filter:none!important}
          .dashboard-shell>.main{min-height:calc(100dvh - 72px - env(safe-area-inset-top))!important;padding-top:32px!important;padding-bottom:max(20px,env(safe-area-inset-bottom))!important;display:flex!important;flex-direction:column!important}
          .dashboard-shell .app-page-content{flex:0 0 auto;gap:16px!important}
          .dashboard-shell>.main>footer{margin-top:20px!important}
          .dashboard-shell .app-page-content>.everitt-cmd-trigger{position:relative!important;z-index:2!important;margin:0!important;background:#fff!important;color:#132433!important;border:1px solid #d5dee5!important;box-shadow:0 10px 24px rgba(19,36,51,.12)!important;transform:none!important}
          .dashboard-shell .app-page-content>.everitt-cmd-trigger *{color:#132433!important}
          .role-portal-shell .metric-grid,.role-portal-shell .role-summary-grid{grid-template-columns:minmax(0,1fr)!important}.role-portal-shell .portal-client-nav{grid-template-columns:repeat(2,minmax(0,1fr))}.role-portal-contractor>.main{padding-bottom:calc(92px + env(safe-area-inset-bottom))!important}
          .contractor-fixed-bottom-nav{position:fixed;left:14px;right:14px;bottom:max(10px,env(safe-area-inset-bottom));z-index:101;display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:6px;padding:7px;border:1px solid rgba(36,63,83,.14);border-radius:16px;background:#fff;box-shadow:0 10px 28px rgba(18,40,56,.18)}
          .contractor-fixed-bottom-nav a{display:flex;min-height:46px;align-items:center;justify-content:center;padding:8px 6px;border-radius:11px;color:#173044!important;font-size:13px;font-weight:700;text-decoration:none}.contractor-fixed-bottom-nav a:hover,.contractor-fixed-bottom-nav a:focus-visible{background:#edf3f6;color:#173044!important}
        }
