...
export function ClientAccessPanel({ jobId, plan, canManage, customerName = null, customerEmail = null, onCustomerEmailChange, onSaveCustomerEmail }: Props) {
  ...
  useEffect(() => {
    if (!canManage) return;
    void (async () => {
      const settingsRes = await fetch('/api/settings/reviews', { cache: 'no-store' });
      const settings = (await settingsRes.json().catch(() => ({}))) as { reviewUrl?: string };
      if (settingsRes.ok) setReviewUrl(settings.reviewUrl || '');
      const { data: jobRow } = await supabase.from('jobs').select('status, assigned_to, assigned_email').eq('id', jobId).maybeSingle();
  ...
  async function grantAccess(emailValue: string) {
    ...
    try { const res = await fetch('/api/clients/grant-access', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: emailValue.trim(), jobId }) }); const json = await res.json(); if (!res.ok) { setMessage(json.error || 'Unable to enable client access.'); return; } setMessage(json.message || 'Client access enabled.'); await loadAccess(); }
    catch { setMessage('Unable to enable customer portal access.'); } finally { setBusy(false); }
  }
  async function revokeAccess(clientUserId: string) { if (busy) return; setBusy(true); setMessage(''); try { const res = await fetch('/api/clients/revoke-access', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ jobId, clientUserId }) }); const json = await res.json(); setMessage(json.message || json.error || 'Updated.'); if (res.ok) await loadAccess(); } catch { setMessage('Unable to turn off customer portal access.'); } finally { setBusy(false); } }
  async function copyLink(token: string | null) { if (!token) { setMessage('The portal link is not available yet.'); return; } try { await navigator.clipboard.writeText(appUrl(`/portal/client?token=${token}`)); setMessage('Portal link copied.'); } catch { setMessage('Unable to copy the portal link.'); } }
  ...
  return <div>
    {canManage && workerEmail ? <div style={{ marginBottom: 18 }}><h3 style={{ marginTop: 0 }}>Worker email</h3><JobEmailComposer buttonLabel="Email worker" recipientEmail={workerEmail} recipientName={workerName} jobId={jobId} docType="message" defaultSubject={workerTemplates[0].subject} defaultBody={workerTemplates[0].body} templates={workerTemplates} metadata={{ recipient_role: 'worker' }} /></div> : null}
    {canManage && jobCompleted ? <div style={{ marginBottom: 18 }}><h3 style={{ marginTop: 0 }}>Customer review</h3>{!normalizedCustomerEmail ? <p className="muted">Add a customer email before asking for a review.</p> : !reviewUrl ? <><p className="muted">Add your Google, Facebook, or website review link before sending.</p><Link className="btn" href={`/settings/reviews?jobId=${encodeURIComponent(jobId)}`}>Set review link</Link></> : <JobEmailComposer buttonLabel="Ask for review" recipientEmail={normalizedCustomerEmail} recipientName={customerName} jobId={jobId} docType="review" defaultSubject="Thank you for choosing us" defaultBody={reviewBody} metadata={{ review_url: reviewUrl }} />}</div> : null}
