...
export function TeamCommandCenter({ enabled }: TeamCommandCenterProps) {
  ...
  const memberSummary = (member: TeamCommandMember): string | null => {
    ...
    if (member.hasUnscheduledAssignments) return t('dashboard.teamCommand.member.assignedDateNotScheduled');
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (active) {
          setError(t('dashboard.teamCommand.loadError'));
          setData(null);
        }
      } finally {
  ...
  const compactMetrics = useMemo(() => {
    ...
    const safeTotals = data?.totals || { activeJobs: 0, overdueJobs: 0 };
  ...
  return (
    <section
      className="card dashboard-today-card team-command-center"
    ...
    >
      ...
      <div style={metricGridStyle}>
        {compactMetrics.map((card) => (
          <Link
            ...
            style={{
              ...metricCardStyle,
              opacity: showInitialLoading ? 0.55 : 1,
      ...
      <div>
        ...
        <div style={{ minHeight: 30 }}>
          ...
          {!loading && !error && !hasMembers ? <p className="muted">{t('dashboard.teamCommand.overview.empty')}</p> : null}
        </div>
