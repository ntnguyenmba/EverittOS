...
export function AddressAutocomplete({ id, label, value, disabled, required, placeholder = 'Start typing an address or enter it manually', className = 'input', onChange, onSelect }: AddressAutocompleteProps) {
  const autoId = useId();
  ...
  useEffect(() => {
    function onDocPointer(event: MouseEvent | TouchEvent) {
      if (!wrapperRef.current?.contains(event.target as Node)) { setOpen(false); setActiveIndex(-1); }
    }
    document.addEventListener('mousedown', onDocPointer);
    document.addEventListener('touchstart', onDocPointer);
    return () => { document.removeEventListener('mousedown', onDocPointer); document.removeEventListener('touchstart', onDocPointer); };
  }, []);
  ...
  useEffect(() => {
    ...
    if (query.length < 3 || disabled) { setSuggestions([]); setState('idle'); setOpen(false); setActiveIndex(-1); return; }

    debounceRef.current = setTimeout(() => {
      void (async () => {
        const cached = readClientCache(query);
        if (cached) { setSuggestions(cached); setState('ready'); setOpen(focusedRef.current); setActiveIndex(-1); return; }
        ...
        try {
          ...
          setSuggestions([]); setState('error'); setOpen(focusedRef.current); setActiveIndex(-1);
        }
      })();
    }, 280);
  ...
  function applySuggestion(suggestion: AddressSuggestion) {
    ...
    onSelect?.(suggestion);
    setSuggestions([]); setOpen(false); setActiveIndex(-1); setState('idle');
  }
  ...
  return (
    <div className="address-autocomplete" ref={wrapperRef} style={{ position: 'relative', width: '100%' }}>
      ...
      <input id={inputId} className={className} value={value} disabled={disabled} required={required} autoComplete="street-address" placeholder={placeholder} role="combobox" aria-expanded={showMenu} aria-controls={listboxId} aria-autocomplete="list" aria-activedescendant={activeIndex >= 0 ? `${listboxId}-option-${activeIndex}` : undefined}
        onChange={(event) => { const next = event.target.value; onChange(next, next.trim() ? structuredAddressFromManual(next) : null); }}
        onFocus={() => { focusedRef.current = true; if (value.trim().length >= 3) setOpen(true); }}
        onBlur={() => { focusedRef.current = false; }} onKeyDown={onKeyDown} />
      {showMenu ? (
        <div id={listboxId} role="listbox" className="address-autocomplete-menu" style={{ position: 'absolute', zIndex: 40, top: 'calc(100% + 4px)', left: 0, right: 0, maxHeight: 300, overflowY: 'auto', border: '1px solid var(--line)', borderRadius: 'var(--radius-md, 12px)', background: 'var(--surface)', color: 'var(--text)', boxShadow: 'var(--shadow-subtle)' }}>
          ...
          <button type="button" onMouseDown={(event) => event.preventDefault()} onClick={useTypedAddress} style={{ display: 'block', width: '100%', padding: '12px', border: 0, background: 'var(--surface-subtle)', color: 'inherit', textAlign: 'left', cursor: 'pointer' }}>
            <strong style={{ display: 'block' }}>Use this address</strong>
