...
export function JobCreator({ onJobCreated }: JobCreatorProps) {
  ...
  const [customerMode, setCustomerMode] = useState<CustomerEntryMode | null>(searchParams.get('customerId') || searchParams.get('customer_id') ? 'existing' : null);
      ...
      <section className="job-create-section">
        ...
        {customerMode === 'existing' ? (
          <div className="client-summary-card" style={{ display: 'grid', gap: 10, marginTop: 12 }}>
            <label><span>{createCopy.searchCustomers}</span>
              <select value={customerId} disabled={loadingCustomers} onChange={(e) => { setCustomerId(e.target.value); setPropertyId(''); }}>
                <option value="">{loadingCustomers ? createCopy.searching : createCopy.selectProperty}</option>
                {customers.map((customer) => <option key={customer.id} value={customer.id}>{customer.contact_name || customer.company_name || customer.email || customer.phone || 'Customer'}</option>)}
              </select>
            </label>
            {customerId && properties.length > 1 ? <p className="muted">{createCopy.multipleProperties}</p> : null}
            <label><span>{createCopy.savedProperties}</span>
              <select value={propertyId} disabled={!customerId || loadingProperties} onChange={(e) => setPropertyId(e.target.value)}>
                <option value="">{!customerId ? createCopy.selectProperty : loadingProperties ? createCopy.searching : properties.length ? createCopy.selectProperty : createCopy.noProperty}</option>
                {properties.map((property) => <option key={property.id} value={property.id}>{property.name ? `${property.name}${property.formatted_address || property.address ? ` · ${property.formatted_address || property.address}` : ''}` : property.formatted_address || property.address || 'Property'}</option>)}
              </select>
            </label>
          </div>
        ) : null}
        {customerMode === 'new' ? (
          <div style={{ display: 'grid', gap: 10, marginTop: 12 }}>
            <label><span>{createCopy.customerName}</span><input value={customerName} onChange={(e) => setCustomerName(e.target.value)} /></label>
            <label><span>{createCopy.email}</span><input type="email" value={customerEmail} onChange={(e) => setCustomerEmail(e.target.value)} /></label>
            <label><span>{createCopy.phone}</span><input value={phone} onChange={(e) => setPhone(e.target.value)} /></label>
            ...
            <AddressAutocomplete id="job-address" label={createCopy.serviceAddress} placeholder={createCopy.addressPlaceholder} value={address} onChange={(formatted, structured) => { setAddress(formatted); setStructuredAddress(structured); }} onSelect={(suggestion) => void resolveTimezoneFromCoords(suggestion.latitude, suggestion.longitude)} />
          </div>
        ) : null}
      </section>
      <section className="job-create-section">
