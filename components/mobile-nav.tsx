...
export function MobileNav({ plan, role: roleProp }: MobileNavProps) {
  ...
  return <header className={`mobile-nav${isFocusedPortal ? ' mobile-nav-focused-portal' : ''}${open ? ' mobile-nav-open' : ''}`} aria-label={t('ux.mobileNavLabel')}>
    ...
    <style>{`
      .mobile-nav{border-bottom:1px solid #d7dfe4;background:#fff}.mobile-nav-bar{min-height:64px;padding:9px 20px;display:flex;align-items:center;justify-content:space-between;gap:14px;box-sizing:border-box}.mobile-nav-brand-logo{color:#173044}.mobile-nav-bar-actions{display:flex;align-items:center;gap:10px}.mobile-nav-language select,.mobile-nav-language button{min-height:44px;border:1px solid #c8d1d7;border-radius:14px;background:#fff;color:#243e51;font-weight:700}.mobile-nav-menu-btn{width:46px;height:46px;display:inline-flex;align-items:center;justify-content:center;border:0;border-radius:14px;background:#f1f4f6;color:#243f53;cursor:pointer}
