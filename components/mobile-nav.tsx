...
export function MobileNav({ plan, role: roleProp }: MobileNavProps) {
  ...
  function openAskEveritt() {
    const trigger = document.querySelector<HTMLButtonElement>('.app-page-content .everitt-cmd-trigger');
    trigger?.click();
  }
    ...
    <header className={`mobile-nav${isFocusedPortal ? ' mobile-nav-focused-portal' : ''}${open ? ' mobile-nav-open' : ''}`} aria-label={t('ux.mobileNavLabel')}>
      <div className="mobile-nav-bar">
        ...
        <div className="mobile-nav-actions">
          {!isFocusedPortal ? (
            <button type="button" className="mobile-nav-search-btn" aria-label="Ask Everitt" onClick={openAskEveritt}>
      ...
      <style>{`
        .mobile-nav{border-bottom:0;background:transparent}.mobile-nav-bar{min-height:52px;padding:6px 0;display:flex;align-items:center;justify-content:space-between;gap:14px;box-sizing:border-box;background:transparent!important;border:0!important;box-shadow:none!important}.mobile-nav-actions{display:flex;align-items:center;gap:8px}.mobile-nav-top-logo{flex:0 0 auto;display:inline-flex;align-items:center;justify-content:center;width:40px;height:40px;border-radius:12px;background:rgba(255,255,255,.96);border:1px solid rgba(255,255,255,.5);box-shadow:0 5px 16px rgba(9,24,35,.12)}.mobile-nav-top-logo .brand-logo-image{display:block;border-radius:8px}.mobile-nav-brand-logo{color:#173044}.mobile-nav-search-btn,.mobile-nav-menu-btn{position:relative;z-index:2147482999;width:42px;height:42px;display:inline-flex;align-items:center;justify-content:center;border:1px solid rgba(210,219,224,.9);border-radius:13px;background:rgba(255,255,255,.96);color:#173044;box-shadow:0 5px 16px rgba(9,24,35,.1);cursor:pointer;pointer-events:auto!important;touch-action:manipulation}.mobile-nav-menu-btn{width:44px;height:44px}.mobile-nav-search-icon{position:relative;width:17px;height:17px;border:2px solid #173044;border-radius:50%;box-sizing:border-box}.mobile-nav-search-icon:after{content:'';position:absolute;width:7px;height:2px;right:-5px;bottom:-2px;border-radius:999px;background:#173044;transform:rotate(45deg);transform-origin:center}.mobile-nav-menu-icon,.mobile-nav-menu-icon:before,.mobile-nav-menu-icon:after{display:block;width:18px;height:2px;border-radius:999px;background:#173044}.mobile-nav-menu-icon{position:relative}.mobile-nav-menu-icon:before,.mobile-nav-menu-icon:after{content:'';position:absolute;left:0}.mobile-nav-menu-icon:before{top:-6px}.mobile-nav-menu-icon:after{top:6px}
        ...
        @media(max-width:760px){.mobile-nav-bar{padding:4px 0}.mobile-nav-top-logo{width:38px;height:38px;border-radius:11px}.mobile-nav-search-btn{width:40px;height:40px}.mobile-nav-menu-btn{width:42px;height:42px}}
        @media(min-width:700px){.mobile-nav-overlay-portal{align-items:stretch;justify-content:flex-end}.mobile-nav-drawer{width:min(390px,calc(100vw - 28px));height:100dvh;max-height:none;border-radius:0;border-top:0}.mobile-nav-drawer-handle{display:none}.mobile-nav-panel .app-nav{grid-template-columns:1fr}.mobile-nav-drawer-footer{grid-template-columns:1fr}}
        @media(max-width:480px){.mobile-nav-panel .app-nav{grid-template-columns:1fr 1fr}.mobile-nav-drawer-footer{grid-template-columns:1fr 1fr}.mobile-nav-logout{grid-column:1/-1}}
      `}</style>
    </header>
  );
}
