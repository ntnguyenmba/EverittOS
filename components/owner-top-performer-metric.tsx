...
export function OwnerTopPerformerMetric() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const periodJobs = filterValidJobsInPeriod((jobsResult.data || []) as RankingJob[], range) as RankingJob[];
      const performerCounts = new Map<string, number>();
      const customerRevenue = new Map<string, { name: string; revenue: number }>();

      for (const job of periodJobs.filter((row) => isCompletedCountableJob(row))) {
        ...
        for (const workerId of Array.from(new Set(workerIds))) {
  ...
  if (!host || !allowed || (performers.length === 0 && customers.length === 0)) return null;
