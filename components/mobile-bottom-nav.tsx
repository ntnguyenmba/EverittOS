...
const bottomCopy = {
  en: { home: 'Home', jobs: 'Jobs', customers: 'Customers', calendar: 'Calendar', more: 'More', workspace: 'Workspace', language: 'Language', logout: 'Log out', close: 'Close' },
  es: { home: 'Inicio', jobs: 'Trabajos', customers: 'Clientes', calendar: 'Calendario', more: 'Más', workspace: 'Espacio de trabajo', language: 'Idioma', logout: 'Cerrar sesión', close: 'Cerrar' },
  vi: { home: 'Trang chủ', jobs: 'Công việc', customers: 'Khách hàng', calendar: 'Lịch', more: 'Thêm', workspace: 'Không gian làm việc', language: 'Ngôn ngữ', logout: 'Đăng xuất', close: 'Đóng' }
} as const;

function Icon({ name }: { name: 'home' | 'jobs' | 'customers' | 'calendar' | 'more' | 'money' }) {
...
function activeFor(pathname: string, href: string) { const path = href.split(/[?#]/)[0]; if (path === '/dashboard' || path === CLIENT_PORTAL_HOME || path === CONTRACTOR_PORTAL_HOME) return pathname === path; return pathname === path || pathname.startsWith(`${path}/`); }

export function MobileBottomNav() {
  const pathname = usePathname() || '/'; const router = useRouter(); const { t, locale } = useTranslation(); const workspace = useWorkspacePlanOptional(); const role = normalizeRole(workspace?.role); const bc = bottomCopy[locale] || bottomCopy.en;
  const [moreOpen, setMoreOpen] = useState(false); const [mounted, setMounted] = useState(false);
  useEffect(() => setMounted(true), []); useEffect(() => setMoreOpen(false), [pathname]);
  useEffect(() => { document.body.classList.toggle('mobile-more-open', moreOpen); return () => document.body.classList.remove('mobile-more-open'); }, [moreOpen]);
  ...
  if (HIDDEN_PREFIXES.some((prefix) => pathname === prefix || pathname.startsWith(`${prefix}/`))) return null; if (!workspace?.plan && !workspace?.role) return null;
  const links = isClientRole(role) ? [{ href: CLIENT_PORTAL_HOME, label: t('portal.common.overview'), icon: 'home' as const }, { href: `${CLIENT_PORTAL_HOME}?tab=jobs`, label: t('portal.common.appointments'), icon: 'jobs' as const }, { href: '#more', label: bc.more, icon: 'more' as const, menu: true }] : isContractorRole(role) ? [{ href: CONTRACTOR_PORTAL_HOME, label: t('portal.contractor.nav.dashboard'), icon: 'home' as const }, { href: `${CONTRACTOR_PORTAL_HOME}#current-jobs`, label: t('portal.contractor.nav.jobs'), icon: 'jobs' as const }, { href: `${CONTRACTOR_PORTAL_HOME}#history`, label: t('portal.contractor.nav.earnings'), icon: 'money' as const }, { href: '#more', label: bc.more, icon: 'more' as const, menu: true }] : [{ href: dashboardPathForRole(role), label: bc.home, icon: 'home' as const }, { href: '/jobs', label: navLabel('/jobs', t, bc.jobs, locale), icon: 'jobs' as const }, { href: '/customers', label: navLabel('/customers', t, bc.customers, locale), icon: 'customers' as const }, { href: '/schedule', label: navLabel('/schedule', t, bc.calendar, locale), icon: 'calendar' as const }, { href: '#more', label: bc.more, icon: 'more' as const, menu: true }];
