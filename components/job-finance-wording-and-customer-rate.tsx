...
export function JobFinanceWordingAndCustomerRate() {
  useEffect(() => {
    const applyGlobalWording = () => {
      replaceWorkerWording(document.body);
      addBookkeepingSibling();
    };

    applyGlobalWording();
    const globalObserver = new MutationObserver(applyGlobalWording);
    globalObserver.observe(document.body, { childList: true, subtree: true, characterData: true });
    window.addEventListener('popstate', applyGlobalWording);

    return () => {
      globalObserver.disconnect();
      window.removeEventListener('popstate', applyGlobalWording);
    };
  }, []);
