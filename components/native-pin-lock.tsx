...
function forceAccountSignIn(): boolean { return window.location.pathname === '/login' && new URLSearchParams(window.location.search).get('force') === '1'; }
function nowNumber(key: string): number { return Number(window.localStorage.getItem(key) || '0') || 0; }
function markActive() { window.localStorage.setItem(LAST_ACTIVE_KEY, String(Date.now())); }

/** Call immediately before Camera, Maps, tel:, or sms: handoffs. */
...
export function NativePinLock() {
  ...
  useEffect(() => {
    const isNative = isNativePlatform(); setNative(isNative); if (!isNative) return;
    setLocale(currentLocale());
    if (!nowNumber(LAST_ACTIVE_KEY)) markActive();

    const shouldLock = () => {
      if (forceAccountSignIn() || !nativePinIsEnabled()) return false;
      const now = Date.now();
      if (now < nowNumber(IGNORE_IDLE_UNTIL_KEY)) return false;
      return now - nowNumber(LAST_ACTIVE_KEY) >= deviceIdleLockMs(window.location.pathname);
    };
    const check = () => { if (shouldLock()) { setPin(''); setError(''); setLocked(true); } };
    const activity = () => { if (!locked && document.visibilityState === 'visible') markActive(); };
    const visibility = () => { if (document.visibilityState === 'visible') check(); };
    const timer = window.setInterval(check, 15_000);
    for (const name of ['pointerdown', 'keydown', 'scroll'] as const) window.addEventListener(name, activity, { passive: true });
    window.addEventListener('everittos:photo-saved', activity);
    document.addEventListener('visibilitychange', visibility);
    check();
    return () => { window.clearInterval(timer); for (const name of ['pointerdown', 'keydown', 'scroll'] as const) window.removeEventListener(name, activity); window.removeEventListener('everittos:photo-saved', activity); document.removeEventListener('visibilitychange', visibility); };
  }, [locked]);
  ...
  async function unlock(event: React.FormEvent) {
    ...
    if (nativePinCooldownRemainingMs() > 0) { setError(c.cooldown); setPin(''); return; }
    const ok = await verifyNativePin(pin);
    if (!ok) { setError(nativePinCooldownRemainingMs() > 0 ? c.cooldown : c.wrong); setPin(''); return; }
    markActive(); setError(''); setLocked(false);
  }
