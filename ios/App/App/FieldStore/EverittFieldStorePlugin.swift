...
public class EverittFieldStorePlugin: CAPPlugin, CAPBridgedPlugin {
    ...
    @objc func listOutbox(_ call: CAPPluginCall) { openDatabase();guard db != nil else { call.resolve(["items":[]]);return };var stmt:OpaquePointer?;sqlite3_prepare_v2(db,"SELECT id,type,record_key,json FROM outbox ORDER BY id ASC;",-1,&stmt,nil);var items:[[String:Any]]=[];while sqlite3_step(stmt)==SQLITE_ROW{let id=sqlite3_column_int64(stmt,0);let type=sqlite3_column_text(stmt,1).map{String(cString:$0)} ?? "";let key=sqlite3_column_text(stmt,2).map{String(cString:$0)} ?? "";let json=sqlite3_column_text(stmt,3).map{String(cString:$0)} ?? "{}";items.append(["id":id,"type":type,"key":key,"json":json])};sqlite3_finalize(stmt);call.resolve(["items":items]) }
